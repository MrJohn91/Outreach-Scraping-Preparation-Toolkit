...
└───────────────────────┬─────────────────────────────┘
                        │ API Calls
┌───────────────────────▼─────────────────────────────┐
│            Apify Platform (Multi-Platform Scrapers)  │
│  ┌─────────────┐  ┌──────────┐  ┌───────────────┐  │
...
- Handle multi-platform scraping (LinkedIn, X, TikTok)
- Route requests to platform-specific actors
- Transform Apify results to application schema

**Supported Platforms:**
| Platform | Apify Actor | Purpose |
|----------|-------------|---------|
| LinkedIn | `harvestapi/linkedin-profile-search` | Find professionals by keyword/location |
| X/Twitter | `kaitoeasyapi/twitter-x-data-tweet-scraper-pay-per-result-cheapest` | Find users by keyword |
| TikTok | `clockworks/tiktok-scraper` | Find creators by keyword |

**Workflow:**
```python
1. Check for APIFY_API_TOKEN
2. Route to platform-specific scraper based on platform parameter
3. Initialize ApifyClient
4. Call platform-specific actor with appropriate input schema
5. Wait for completion
6. Fetch and transform results to standardized schema
7. Return results
```
...
```python
{
  "id": str,              # Unique identifier (e.g., "li_abc123")
  "name": str,            # Person's name
  "role": str,            # Job title or @handle
  "company": str,         # Company name (LinkedIn) or empty
  "platform": str,        # "LinkedIn", "X", or "TikTok"
  "contact_link": str,    # Profile URL
  "region": str,          # Location
  "notes": str,           # Search keyword
  "followers": int,       # Follower count
  "verified": bool,       # Verification status (X, TikTok)
  "headline": str,        # LinkedIn headline
  "bio": str,             # Profile bio/summary
  "industry": str         # Industry (LinkedIn)
}
```
...
### 2. Multi-Platform Web Scraping with Apify

**Purpose:** Extract people/profile data from LinkedIn, X (Twitter), and TikTok

**Features:**
- Real scraping via Apify API
- Platform-specific actors and input schemas
- Configurable max results
- Standardized output format across platforms

**Platform Actors:**

| Platform | Actor | Input Example |
|----------|-------|---------------|
| LinkedIn | `harvestapi/linkedin-profile-search` | `{searchQuery: "AI Founder", locations: ["Berlin"], maxItems: 20}` |
| X/Twitter | `kaitoeasyapi/twitter-x-data-tweet-scraper-pay-per-result-cheapest` | `{twitterContent: "AI startup", maxItems: 60, queryType: "Top"}` |
| TikTok | `clockworks/tiktok-scraper` | `{searchQueries: ["tech founder"], searchSection: "/user", maxProfilesPerQuery: 20}` |
