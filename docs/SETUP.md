...
The toolkit requires API tokens for web scraping functionality.
...
#### Get Your Exa.ai API Key (Required for LinkedIn Search)
...
```bash
# Required for all scraping
APIFY_API_TOKEN=your_token_here

# Required for LinkedIn search (Exa.ai powered)
...
**Solution:**
1. Try broader search terms (e.g., "Founder" instead of "AI Blockchain Founder")
2. Use well-known locations (e.g., "San Francisco" instead of "SF Bay Area")
3. Increase `max_results` parameter
4. Check Apify account has sufficient credits
5. Try different platforms (LinkedIn, X, TikTok) to see which has better coverage

**Problem:** "APIFY_API_TOKEN environment variable is required"

**Solution:**
This error means:
- `APIFY_API_TOKEN` is not set in `.env` file
- Token is set to placeholder value `your_apify_api_token_here`

To fix, ensure you have a valid Apify token configured in your `.env` file.

**Problem:** "EXA_API_KEY environment variable is required for LinkedIn search"

**Solution:**
This error means:
- `EXA_API_KEY` is not set in `.env` file
- Key is set to placeholder value `your_exa_api_key_here`

To fix:
1. Sign up at [exa.ai](https://exa.ai)
2. Get your API key from Dashboard > API Keys
3. Add it to your `.env` file: `EXA_API_KEY=your_actual_key_here`
...
- See [COST_ESTIMATION.md](./COST_ESTIMATION.md) for pricing details
